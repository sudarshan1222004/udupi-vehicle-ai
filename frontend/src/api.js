...
      lng: parseFloat(item.lon) // Changed from 'lon' to 'lng' for consistency
    }));
  } catch (err) {
...
  try {
    // Ensure both coordinates exist before calling OSRM
...
    // Fallback: Calculate straight line distance
    // We must ensure 'end' object has valid coordinates before proceeding
...
    return {
      coordinates: [[start.lat, start.lng], [endLat, endLng]], // FIXED: Using safe endLat/endLng
      distance_km: dist,
...
// 4. GET AI PRICE PREDICTION (CRITICALLY CHECKED KEYS)
export const getPricePrediction = async (pickup, drop, distance) => {
  try {
    const res = await axios.post('http://127.0.0.1:8000/predict_ride', {
      pickup_lat: pickup.lat, // CORRECTED KEY
      pickup_lon: pickup.lng, // CORRECTED KEY
      dropoff_lat: drop.lat, // CORRECTED KEY
      dropoff_lon: drop.lng, // CORRECTED KEY
      distance_km: parseFloat(distance) // CORRECTED KEY
    });
    return res.data;
  } catch (err) {
    console.error("Backend Error:", err);
    alert("Backend is offline or returning a hard error! Please run: uvicorn main:app --reload");
    return null;
  }
};
